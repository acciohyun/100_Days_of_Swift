...
class ViewController: UIViewController {
    ...
    func loadLevel(){
        var clueString = "" //level's clues
        var solutionString = "" // # of letters in each answer
        var letterBits = [String]() //all letter groups
        ...
        if let levelFileURL = Bundle.main.url(forResource: "level\(level)", withExtension: "txt"){
            if let levelContents = try? String(contentsOf: levelFileURL){
                ...
                for (index, line) in lines.enumerated(){
                    let parts = line.components(separatedBy: ": ") //puts separated components into an array
                    let answer = parts[0]
                    let clue = parts[1]
